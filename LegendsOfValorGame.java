...
public class LegendsOfValorGame {
    ...
    private HeroAction chooseHeroAction(HeroUnit unit) {
        ...
        switch (choice) {
            case 1: return new MoveAction();
            case 2: return new AttackAction();
            case 3: return new TeleportAction();
            case 4: return new RecallAction();
            default: return new PassAction();
        }
    }
    ...
    public boolean canMonsterMoveTo(MonsterUnit unit, Position dest) {
        ...
        if (tile.getMonster() != null) return false; // no stacking monsters
        if (tile.getHero() != null) return false;    // heroes can block
        return true;
    }
    ...
    public List<Position> validTeleportDestinations(HeroUnit unit, HeroUnit target) {
        ...
        for (int[] d : dirs) {
            ...
            if (!board.inBounds(candidate)) continue;
            if (!canHeroMoveTo(unit, candidate)) continue;

            // Cannot teleport to a tile "ahead" of the target hero
            if (candidate.row < targetPos.row) continue;
    ...
    private void regenHeroes() {
        // At end of each round, each alive hero regains ~10% HP. :contentReference[oaicite:3]{index=3}
        for (HeroUnit h : heroes) {
            Hero hero = h.getHero();
            if (hero.getHp() > 0) {
    ...
    private void maybeSpawnNewMonsters() {
        if (roundNumber % spawnFrequency != 0) return;
    ...
    private int indexOfHero(Hero hero) {
        for (int i = 0; i < heroes.size(); i++) {
            if (heroes.get(i).getHero() == hero) return i;
        }
        return -1;
    }
    ...
    private int indexOfMonster(Monster monster) {
        for (int i = 0; i < monstersOnBoard.size(); i++) {
            if (monstersOnBoard.get(i).getMonster() == monster) return i;
        }
        return -1;
    }
    ...
    private String terrainSymbol(LegendsTile tile) {
        switch (tile.getTerrainType()) {
            case HERO_NEXUS:    return "N";
            case MONSTER_NEXUS: return "N";
