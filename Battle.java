...
/**
 * Handles the full turn-based battle system between a party of heroes and a list of monsters.
 * Manages turn order, actions, damage calculation, rewards, and win/lose conditions.
 */
public class Battle {
    private Party party;
    ...
    private boolean heroCastSpell(Hero h) {
        ...
        switch (currentWeather) {
            case RAINY:
                if (spell.getType() == SpellType.LIGHTNING) weatherBonus = 1.25;
                break;
            case SUNNY:
                if (spell.getType() == SpellType.FIRE) weatherBonus = 1.25;
                break;
            case SNOWY:
                if (spell.getType() == SpellType.ICE) weatherBonus = 1.25;
                break;
            case STORM:
        ...
        switch (spell.getType()) {
            case ICE: m.reduceDamage(0.1); break;
            case FIRE: m.reduceDefense(0.1); break;
            case LIGHTNING: m.reduceDodge(0.1); break;
        }
        view.showMessage(h.getName() + " cast " + spell.getName() + " on " + m.getName() + " for " + (int)dmg + " damage.");
        return true;
    }
    ...
    private boolean heroUsePotion(Hero h) {
        ...
        switch (p.getStat()) {
            case HP: h.hp += p.getAmount(); break;
            case MP: h.mana += p.getAmount(); break;
            case STRENGTH: h.strength += p.getAmount(); break;
            case DEXTERITY: h.dexterity += p.getAmount(); break;
            case AGILITY: h.agility += p.getAmount(); break;
        }
    ...
    private void rewardHeroes() {
        int aliveCount = 0;
        for (Hero h : party.getHeroes()) if (h.isAlive()) aliveCount++;
        int exp = monsters.size() * 2;
        for (Hero h : party.getHeroes()) {
            if (h.isAlive()) {
                h.gainExperience(exp);
                double goldGain = 0;
                for (Monster m : monsters) goldGain += m.getLevel() * 100;
    ...
    private Monster chooseMonster() {
        int aliveCount = 0;
        for (Monster m : monsters) if (m.isAlive()) aliveCount++;
    ...
    private Hero randomLivingHero() {
        ...
        for (Hero h : party.getHeroes()) if (h.isAlive()) alive.add(h);
